...
class MyHashSet
  ...
  def union(set2)
    ...
    @store.each_key { |key| new_set.insert(key) }
  ...
  def intersect(set2)
    new_set = MyHashSet.new
    @store.each_key { |key| new_set.insert(key) if set2.include?(key)}
    new_set
  end

  def minus(set2)
    new_set = MyHashSet.new
    @store.each_key { |key| new_set.insert(key) unless set2.include?(key)}
    new_set
  end
end
